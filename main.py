...
def saveRecord():
    global data
    data.insertRecord(item_name=item_name.get(), item_amt=item_amt.get(), purchase_date=transaction_date.get())
...
def fetch_records():
    f = data.fetchRecord('select rowid, * from expenses')
    global count
    for rec in f:
...
def totalBalance():
    f = data.fetchRecord(query="Select sum(item_amt) from expense_record")
    for i in f:
        for j in i:
...
f=('Times new Roman',14)
namevar=StringVar()
amtvar=IntVar()  # Define amtvar here
dopvar=StringVar()

# Entry widgets
item_name = Entry(f1, font=f, textvariable=namevar)
item_amt = Entry(f1, font=f, textvariable=amtvar)  # Use amtvar here
